...
columnStringLeft = 'category_-1'
#columnStringRight = 'category_NA'
#columnStringLeft = 'segment_'
columnStringRight = 'segment_9'
X = df.loc[:,columnStringLeft:columnStringRight]
y = df.loc[:,'clicks']
#y = df.loc[:,'installs']
...
model = LogisticRegression(class_weight = 'balanced', penalty = 'l1').fit( X_train, y_train )
#model.coef_
...
# CREATE A LIST OF THE CATEGORIES WITH WEIGHTED COEFFICIENTS OF ZERO
rareColumns = []
for i in range(X.sum().count()):
    if model.coef_[0][i] == 0:
        rareColumns.append(list(X)[i])
        
###############################################################################
        
# Test model on a specific row

rowNumber = 6
testRow = df.loc[rowNumber,columnStringLeft:columnStringRight]
testArray = testRow.as_matrix()
